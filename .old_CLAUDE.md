...
**Escuchowsky** es una app de descubrimiento musical que cruza el historial de Last.fm del usuario con colecciones para mostrar qué álbumes no ha escuchado aún.

Dos aplicaciones Flask independientes, cada una con su propio dominio y contenedor:

- **escuchowsky** (`app_genres.py`, port 5001) — mustlisten: colecciones y árbol de géneros
...
3. Unheard albums highlighted against curated collections
...
### Database (`db/must_hear.db`)

SQLite with four tables: `artists`, `albums`, `collections`, `collection_albums`. Schema in `db/lastfm_normalized.sql`. Mounted as a volume from `./db`.

`app_genres.py` also reads `db/genres.json` (hierarchical genre tree) at startup.

### Entrypoint

`entrypoint_escuchowsky.sh` runs `app_genre_mermaid.py` first (generates `genre_tree.html` from `genres.json`), then starts gunicorn (2 workers × 4 threads).

...
// ── Cover enrichment for collection albums without MBID/cover ────────────────
function enrichMissingCovers() {
  if (_enrichEs) { _enrichEs.close(); _enrichEs = null; }
  // Collect up to 50 albums without cover (in rank order = allAlbums order)
  const toEnrich = [];
  ...
  _enrichEs.onmessage = (e) => {
    ...
    // Update allAlbums so filters/re-renders use the enriched cover
    allAlbums[albumIdx].cover = msg.cover_url;
    if (msg.mbid) allAlbums[albumIdx].mbid = msg.mbid;
    // Also persist in collCache so switching back to this collection reuses it
    ...
    // Update the card DOM directly (no full re-render needed)
...
async function loadAndRender(slug) {
  // Abort any previous collection load, cover enrichment and in-flight image requests
  if (_enrichEs) { _enrichEs.close(); _enrichEs = null; }
...
function applyCollection(slug) {
  ...
  buildDecadePills();
  renderGrid();
  enrichMissingCovers();  // lazy MusicBrainz lookup for albums without cover/MBID
}

...
function cslug(s) {
  return "genre_" + s.replace(/-/g, "_");
}
function isScraped(s) {
  return !!CHARTS[cslug(s)];
}
...
for (const [slug, pd] of Object.entries(PANEL_DATA)) {
  for (const alb of pd.albums || []) {
    ...
    if (!ARTIST_INDEX[key])
      ARTIST_INDEX[key] = { display: alb.artist, genres: [] };
    if (!ARTIST_INDEX[key].genres.includes(slug))
      ARTIST_INDEX[key].genres.push(slug);
  }
}
...
function render() {
  ...
  function linkPath(d) {
    const sx = d.source.y + NODE_W,
      sy = d.source.x + NODE_H / 2;
    const tx = d.target.y,
      ty = d.target.x + NODE_H / 2;
    const mx = (sx + tx) / 2;
  ...
  const NODE_BG = (depth, scraped) =>
    depth === 0 ? "#2a1e00" : scraped ? "#1a1300" : "#161616";
  const NODE_STR = (depth, scraped) =>
    depth === 0 ? C_ACCENT : scraped ? "#4a3800" : "#2a2a2a";
  const NODE_STW = (depth) => (depth === 0 ? 2 : 1);
  const TEXT_CLR = (depth, scraped) =>
    depth === 0 ? C_ACCENT : scraped ? "#e0e0e0" : "#666";
  ...
  enter
    ...
    .style("cursor", (d) =>
      (d.data.node._raw.c || []).length > 0 ? "pointer" : "default",
    )
    ...
    .on("mouseout", function (e, d) {
      d3.select(this).attr(
        "stroke",
        NODE_STR(d.depth, isScraped(d.data.node.slug)),
      );
    })
    .on("click", (e, d) => {
      e.stopPropagation();
  ...
  const expandG = enter
    ...
    .style("display", (d) =>
      (d.data.node._raw.c || []).length > 0 ? null : "none",
    )
    .style("cursor", "pointer")
    .on("click", (e, d) => {
      e.stopPropagation();
  ...
  update.each(function (d) {
    ...
    d3.select(this)
      ...
      .attr(
        "stroke",
        isHl ? "#ff9" : NODE_STR(d.depth, isScraped(d.data.node.slug)),
      )
...
function showPanel(slug) {
  ...
  if (hasChart) {
    html += `<a class="panel-title" href="charts/${cs}/index.html" target="_blank">${data.name || slug}</a>`;
  } else {
...
function _renderPanelPage() {
  ...
  const maxPage = Math.max(
    0,
    Math.ceil(Math.min(total, 15) / PANEL_PER_PAGE) - 1,
  );
  ...
  if (info)
    info.textContent = `Pág.${_panelPage + 1}/${maxPage + 1} · ${Math.min(total, 15)} vídeos`;
...
function _openIDB() {
  return new Promise((res, rej) => {
    ...
    req.onupgradeneeded = (e) =>
      e.target.result.createObjectStore("sessions", { keyPath: "user" });
...
async function loadHeardFromIdb() {
  try {
    ...
    console.warn("loadHeardFromIdb:", e);
  }
}
...
async function loadUser() {
  ...
  _scrobbleEs = new EventSource(
    `/api/scrobbles?user=${encodeURIComponent(username)}`,
  );
    ...
    if (msg.done) {
      ...
      try {
        ...
        await new Promise((res, rej) => {
          ...
          tx.oncomplete = res;
          tx.onerror = (err) => rej(err.target.error);
        });
      } catch (err) {
        console.warn("IDB save:", err);
      }
...
document
  .getElementById("panelPrev")
  .addEventListener("click", () => panelAlbPage(-1));
document
  .getElementById("panelNext")
  .addEventListener("click", () => panelAlbPage(1));
